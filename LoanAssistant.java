			...
			public void actionPerformed(ActionEvent e) {
				...
				if (!isValid(interest.getText())) {
					JOptionPane.showConfirmDialog(null, "Invalid or empty Interest Rate field \nPlease correct",
							"Interest Rate Input Error", JOptionPane.DEFAULT_OPTION, JOptionPane.INFORMATION_MESSAGE);
					interest.setText("");
				...
				if (byMonth.isVisible() && !isValid(payments.getText())) {
					JOptionPane.showConfirmDialog(null, "Invalid or empty Payment No. field \nPlease correct",
							"Payment Number Input Error", JOptionPane.DEFAULT_OPTION, JOptionPane.INFORMATION_MESSAGE);
					payments.setText("");
				...
				if (byNo.isVisible() && !isValid(months.getText())) {
					JOptionPane.showConfirmDialog(null, "Invalid or empty Monthly Payment field \nPlease correct",
							"Monthly Payment Input Error", JOptionPane.DEFAULT_OPTION, JOptionPane.INFORMATION_MESSAGE);
					months.setText("");
					months.requestFocus();
					return;
				}
				...
				if (byMonth.isVisible()) {
					...
					if (payment > balance || payment == 0) {
						JOptionPane.showConfirmDialog(null,
								"Invalid No. of Payment field \nPlease provide proper value",
								"Number of Payment Input Error", JOptionPane.DEFAULT_OPTION,
					...
					if (month > balance || month == 0) {
						JOptionPane.showConfirmDialog(null,
								"Invalid Monthly Payment field \nPlease provide proper value",
								"Monthly Payment Input Error", JOptionPane.DEFAULT_OPTION,
								JOptionPane.INFORMATION_MESSAGE);
						months.setText("");
						months.requestFocus();
						return;
					...
					if (month <= balance * rat + 1.0) {
						if (JOptionPane.showConfirmDialog(null,
								"Minimum payment must be Rs. "
										+ new DecimalFormat("0.00").format((int) (balance * rat + 1.0))
							...
							JOptionPane.showConfirmDialog(null, "Enter Monthly Payment field again",
									"Monthly Payment Input Error", JOptionPane.DEFAULT_OPTION,
									JOptionPane.INFORMATION_MESSAGE);
							months.setText("");
							months.requestFocus();
					...
					if (rat == 0)
						payment = (int) (balance / month);
					else
						payment = (int) ((Math.log(month) - Math.log(month - balance * rat)) / Math.log(1 + rat));
				...
				result.append("\n\n" + String.valueOf(payment - 1) + " Payments of Rs. "
						+ new DecimalFormat("0.00").format(month));
				result.append("\n" + "Final Payment of Rs. " + new DecimalFormat("0.00").format(last));
				result.append("\n" + "Total Payment of Rs. "
						+ new DecimalFormat("0.00").format((payment - 1) * month + last));
				result.append("\n" + "Interest Paid : Rs. "
						+ new DecimalFormat("0.00").format((payment - 1) * month + last - balance));
